...
public class _04_Three_Dimenstional_Array_Using_in_WebSite2 {

	public String[][][] Read_Excel() throws IOException, BiffException {
		FileInputStream fileInputStream_obj = new FileInputStream("C:\\WebDriver\\TESTING FILES\\Data Order in xls.xls");
		...
		String testDataString[][][] = new String [rows-1][columns][columns]; // Creating a 2 Dimensional Matrix Array(TestData Array)
																	// rows-1 means 5-1 = 4 Heading not included to create TestData Array 
																	// columns = 2 TestData Array 	
		for(int i =1;i<rows;i++) {
			for(int j =0;j<columns;j++) {
				//first column, next row
				testDataString[i-1][j][j] = sheet_obj.getCell(j,i).getContents(); //Sending a Cell Content to 2 Dimensional Matrix Array(TestData Array)
			}
		}
